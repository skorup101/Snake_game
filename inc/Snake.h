...
public:
	...
	void handleBoarderCrossing() {
		switch (currentDirection) {
			case Up:	if(snakeBody[0].getY() < 0) snakeBody[0] = SnakeSegment(snakeBody[0].getX(), WINDOW_HEIGHT - SEGMENT_SIZE);	break;
			case Down:	if(snakeBody[0].getY() == WINDOW_HEIGHT) snakeBody[0] = SnakeSegment(snakeBody[0].getX(), 0);				break;
			case Right: if(snakeBody[0].getX() == WINDOW_WIDTH) snakeBody[0] = SnakeSegment(0, snakeBody[0].getY());				break;
			case Left:	if(snakeBody[0].getX() < 0) snakeBody[0] = SnakeSegment(WINDOW_WIDTH - SEGMENT_SIZE, snakeBody[0].getY());	break;
			default:																												break;
			}
	}
	...
	bool crashed() {
		for (size_t i = 1; i < snakeBody.size(); ++i) {
			if (getHead() == snakeBody[i]) return true;
		}
		return false;
	}
...
private:
	...
	coord randCoordX() { //coord within window's range in SEGMENTs net
		return ((rand() % WINDOW_WIDTH)/ SEGMENT_SIZE) * SEGMENT_SIZE;
	}

	coord randCoordY() { //coord within window's range in SEGMENTs net
